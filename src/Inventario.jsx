...
// -------------------------------------------------
// Características:
// 1) Columna de "Ubicación" en Componentes (Bodega / Equipo / Tránsito / Reserva) con centro explícito
// 2) Vista de Equipos completas con componentes actuales
// 3) Ensamblar / Desarmar mediante RPC (validaciones en BD)
// 4) Crear equipos (autocódigo EQP-####) y asignación inicial opcional
// 5) Alta de componentes + bodega inicial (centro) opcional
// 6) Búsqueda + Filtro por Tipo (chips)
// 7) NUEVO: "Mover a bodega de…" (usa rpc_mov_crear + rpc_mov_recepcionar)
// 8) NUEVO: Estado de Taller para equipos (check-in / check-out)
//
// Requisitos:
// - Supabase: tablas base + vistas v_componente_ubicacion_actual, v_usuario_ubicacion_actual.
// - RPCs: rpc_equipo_crear, rpc_ensamblar_componente, rpc_desarmar_componente, rpc_mov_crear, rpc_mov_recepcionar.
// - Para taller (equipos): ver patch SQL que acompaño en el chat: equipo_taller_historial + rpc_equipo_taller_checkin/checkout + vista v_equipo_taller_actual.
...
async function fetchEquipos({ centro_id = null, rol = 'oficina' }) {
  ...
  // Mapa de taller (si existe la vista)
  let tallerMap = {}
  try {
...
async function createComponente({ tipoId, estadoId, serie, fecha, codigoManual }) {
  const payload = [{ tipo_componente_id: tipoId, estado_componente_id: estadoId, fecha_ingreso: fecha || new Date().toISOString().slice(0,10), serie, codigo: codigoManual || null }]
  const { data, error } = await supabase.from('componentes').insert(payload).select('id, codigo').single()
  if (error) throw error
  return data
}

async function setBodegaInicial({ componenteId, centroId, fecha }) {
  const { data, error } = await supabase.rpc('fn_componente_bodega_set', { p_componente_id: componenteId, p_centro_id: centroId, p_fecha: fecha || null })
  if (error) throw error
  return data
}
...
export default function Inventario() {
  ...
  const [tipoChips, setTipoChips] = useState([]) // ids seleccionados
  ...
  // Crear componente -----------------------------------------------------
  async function openModalAddComp() {
  ...
  // Mover a bodega de… ---------------------------------------------------
  function openModalMover(comp) {
    setCompAMover(comp); setDestCentroId(''); setOpenMover(true); setErrorMsg(''); setOkMsg('')
  }
    ...
    try {
      ...
      if (compAMover.ubic_tipo === 'bodega') {
        const { data: origenRow } = await supabase
          .from('componente_bodega_historial')
          .select('centro_id')
          .eq('componente_id', compAMover.id)
          .is('fecha_fin', null)
          .maybeSingle()
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={`Ensamblar en ${modalEquipo?.codigo || ''}`}
        footer={<><button onClick={() => setModalOpen(false)} className="px-3 py-2 rounded-xl bg-zinc-800 text-zinc-100 border border-zinc-700 hover:bg-zinc-700">Cancelar</button>
      ...
      <Modal open={openCrear} onClose={() => setOpenCrear(false)} title="Crear equipo"
        footer={<><button onClick={() => setOpenCrear(false)} className="px-3 py-2 rounded-xl bg-zinc-800 text-zinc-100 border border-zinc-700 hover:bg-zinc-700">Cancelar</button>
      ...
      <Modal open={openAddComp} onClose={() => setOpenAddComp(false)} title="Agregar componente"
        footer={<><button onClick={() => setOpenAddComp(false)} className="px-3 py-2 rounded-xl bg-zinc-800 text-zinc-100 border border-zinc-700 hover:bg-zinc-700">Cancelar</button>
      ...
      <Modal open={openMover} onClose={() => setOpenMover(false)} title={`Mover ${compAMover?.codigo || ''} a bodega de…`}
        footer={<><button onClick={() => setOpenMover(false)} className="px-3 py-2 rounded-xl bg-zinc-800 text-zinc-100 border border-zinc-700 hover:bg-zinc-700">Cancelar</button>
      ...
      <Modal open={openTaller} onClose={() => setOpenTaller(false)} title={equipoTaller?.en_taller ? 'Retornar de taller' : 'Enviar a taller'}
        footer={<><button onClick={() => setOpenTaller(false)} className="px-3 py-2 rounded-xl bg-zinc-800 text-zinc-100 border border-zinc-700 hover:bg-zinc-700">Cancelar</button>
